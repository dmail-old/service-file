...
export function suite(add){
	...
	add("status 200 for existing file", function(){
		return this.resolveWith(rest.get('200.txt'), {status: 200});
	});

	add("status 403 for EPERM", function(){
		return this.resolveWith(rest.get('EPERM'), {status: 403});
	});
	...
	add("status 404 for ENOENT", function(){
		return this.resolveWith(rest.get('ENOENT'), {status: 404});
	});

	add("status 503 for EBUSY", function(){
		return this.resolveWith(rest.get('EBUSY'), {status: 503});
	...
	add("response body is file content", function(){
		return this.resolveWith(rest.get('200.txt').then(function(response){
